...
def decorator(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        # Prevent browser from caching private pages
...
return (
    ...
    "(only for accounts with a password set)."
)
```
...
@app.route(
"/update-status/[int:lead_id](int:lead_id)",
methods=["POST"]
)
@login_required(
role=[
...
@app.route(
"/delete/[int:lead_id](int:lead_id)",
methods=["POST"]
)
@login_required(
role=[
...
# Prevent caching of private export
response.headers["Cache-Control"] = (
    "no-store, "
    "no-cache, "
    "must-revalidate, "
